...
const ALLOWED_PURPOSES = new Set(['task_evidence']);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const purpose =
      typeof purposeValue === 'string' && purposeValue.trim() ? purposeValue.trim() : 'task_evidence';
    ...
    const fileName = fileValue.name?.trim() || `task-evidence-${Date.now()}.bin`;
