...
export default function ProfileScreen() {
  ...
  const { logout, updateSessionUser } = useAgentSession();
  const [profile, setProfile] = useState<AgentProfileUser | null>(null);
  ...
  const hydrateForm = useCallback((user: AgentProfileUser) => {
    setForm({
  ...
  const loadProfile = useCallback(
    async (activeRef: { current: boolean } = { current: true }) => {
      ...
      try {
        ...
        if (!activeRef.current) return;
        const message = toAgentErrorMessage(loadError, t('profile.unavailableFallback'));
        setError(message);
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      const message = toAgentErrorMessage(saveError, t('profile.unavailableFallback'));
      setError(message);
      if (isAgentAuthError(saveError)) {
        await logout();
      }
    } finally {
      setSaving(false);
  ...
  async function handleChangePassword() {
    ...
    try {
      ...
      if (isAgentAuthError(changePasswordError)) {
        await logout();
      }
    } finally {
  ...
  return (
    <RequireAgentAuth>
      <AppPage title={t('common.profile')} topBarMode="back" backHref="/(tabs)">
        {loading ? (
              ...
              <View style={styles.sectionHeader}>
                ...
                {!editing ? (
                  <Pressable
                    ...
                    <Text style={[styles.inlineActionText, { color: palette.primary }]}>{t('common.edit')}</Text>
                  </Pressable>
                ) : null}
              </View>
                ...
                <View style={styles.formStack}>
                  ...
                  <View style={styles.actionsRow}>
                    <Pressable
                      ...
                      <Text style={[styles.secondaryButtonText, { color: palette.headline }]}>{t('common.cancel')}</Text>
                    </Pressable>
...
function getDisplayName(profile: AgentProfileUser) {
  return [profile.firstName, profile.lastName].filter(Boolean).join(' ') || profile.username || 'Agent';
}
